defmodule ViaEstimation.Matrix do
  ...
  @spec tuple_to_matrex(tuple(), integer(), integer()) :: struct()
  def tuple_to_matrex(tuple_mat, rows, columns) do
    m = Matrex.zeros(rows, columns)

    {_, matrix} =
      Enum.reduce(Tuple.to_list(tuple_mat), {0, m}, fn val, {index, acc} ->
        row = floor(index / columns)
        column = index - row * columns
        {index + 1, Matrex.set(acc, row + 1, column + 1, val)}
      end)
